...
def get_python_inc(plat_specific=0, prefix=None):
    ...
    if IS_PYPY:
        ...
        python_dir = f'python{get_python_version()}{build_flags}'
...
def get_python_lib(plat_specific=0, standard_lib=0, prefix=None):
    ...
    if os.name == "posix":
        if plat_specific or standard_lib:
            ...
            # Pure Python
            libdir = "lib"
        libpython = os.path.join(prefix, libdir, f"python{get_python_version()}")
...
def customize_compiler(compiler):
    ...
    cpp = os.environ['CPP'] if 'CPP' in os.environ else f"{cc} -E"
...
def expand_makefile_vars(s, vars):
    ...
    while m := _findvar1_rx.search(s) or _findvar2_rx.search(s):
        (beg, end) = m.span()
...
def get_config_vars(*args):
    ...
    if _config_vars is None:
        if func := globals().get(f"_init_{os.name}"):
            func()
        else:
            _config_vars = {}
